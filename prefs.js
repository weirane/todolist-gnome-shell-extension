...
function buildPrefsWidget() {
    ...
    // Shortcut Box
    let shortcut_hbox =
        new Gtk.HBox({margin_left: 20, margin_top: 10, spacing: 6});
    ...
    let cellrend = new Gtk.CellRendererAccel(
        {'editable': true, 'accel-mode': Gtk.CellRendererAccelMode.GTK});
    ...
    settings.connect('changed::shortcut-keybind', (k, b) => {
        let row = model.get(0);
    ...
    let switch_hbox =
        new Gtk.HBox({margin_left: 20, margin_top: 10, spacing: 6});
    let onoff =
        new Gtk.Switch({active: settings.get_boolean(COPY_TO_CLIPBOARD_KEY)});
    switch_hbox.pack_start(
        new Gtk.Label({label: clipboard_str, use_markup: true, xalign: 0}),
        true, true, 0);
    switch_hbox.pack_end(onoff, false, false, 0);
    settings.connect('changed::' + COPY_TO_CLIPBOARD_KEY, (k, b) => {
        onoff.set_active(settings.get_boolean(b));
    });
    onoff.connect('notify::active', (w) => {
